# 📊 Appointment & Follow-Up Flow Diagrams

## 📋 Table of Contents

1. [Regular Appointment Lifecycle](#regular-appointment-lifecycle)
2. [Follow-Up Appointment Flow](#follow-up-appointment-flow)
3. [Recurring Appointment Series](#recurring-appointment-series)
4. [Appointment Chain Structure](#appointment-chain-structure)
5. [State Transitions](#state-transitions)

---

## 🔄 Regular Appointment Lifecycle

```
┌─────────────────────────────────────────────────────────────────┐
│                    REGULAR APPOINTMENT LIFECYCLE                 │
└─────────────────────────────────────────────────────────────────┘

                    [PATIENT/DOCTOR REQUEST]
                              │
                              ▼
                    ┌─────────────────────┐
                    │  VALIDATE REQUEST   │
                    │  - RBAC Check       │
                    │  - Business Rules   │
                    │  - Input Validation │
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  CHECK CONFLICTS    │
                    │  - Doctor Schedule  │
                    │  - Resource Avail   │
                    │  - Patient Appts    │
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  CREATE APPOINTMENT │
                    │  Status: SCHEDULED  │
                    │  parentId: null     │
                    │  isFollowUp: false  │
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  QUEUE OPERATIONS   │
                    │  - Notifications     │
                    │  - Cache Update      │
                    │  - Event Emission    │
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  HIPAA AUDIT LOG    │
                    └─────────────────────┘
                              │
                              ▼
                    [APPOINTMENT CREATED]
                              │
                              ▼
                    ┌─────────────────────┐
                    │  STATUS: SCHEDULED │
                    └─────────────────────┘
                              │
                    ┌─────────┴─────────┐
                    │                   │
                    ▼                   ▼
          [CONFIRMED]          [CHECKED_IN]
                    │                   │
                    └─────────┬─────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  STATUS: IN_PROGRESS │
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  STATUS: COMPLETED  │
                    │  completedAt: now() │
                    └─────────────────────┘
                              │
                    ┌─────────┴─────────┐
                    │                   │
                    ▼                   ▼
        [NO FOLLOW-UP]        [FOLLOW-UP REQUIRED]
                    │                   │
                    │                   ▼
                    │          ┌─────────────────────┐
                    │          │ CREATE FOLLOW-UP   │
                    │          │ PLAN               │
                    │          └─────────────────────┘
                    │                   │
                    │                   ▼
                    │          ┌─────────────────────┐
                    │          │ AUTO-SCHEDULE (if    │
                    │          │ date provided)      │
                    │          └─────────────────────┘
                    │
                    ▼
            [END OF FLOW]
```

---

## 🔁 Follow-Up Appointment Flow

```
┌─────────────────────────────────────────────────────────────────┐
│                    FOLLOW-UP APPOINTMENT FLOW                    │
└─────────────────────────────────────────────────────────────────┘

                    [APPOINTMENT COMPLETED]
                              │
                              ▼
                    ┌─────────────────────┐
                    │  followUpRequired?  │
                    └─────────────────────┘
                              │
                    ┌─────────┴─────────┐
                    │                   │
                    ▼                   ▼
              [NO]                 [YES]
                    │                   │
                    │                   ▼
                    │          ┌─────────────────────┐
                    │          │ CREATE FOLLOW-UP   │
                    │          │ PLAN               │
                    │          │ - appointmentId     │
                    │          │ - scheduledFor     │
                    │          │ - followUpType     │
                    │          │ - status: scheduled│
                    │          └─────────────────────┘
                    │                   │
                    │                   ▼
                    │          ┌─────────────────────┐
                    │          │ followUpDate        │
                    │          │ provided?           │
                    │          └─────────────────────┘
                    │                   │
                    │          ┌─────────┴─────────┐
                    │          │                   │
                    │          ▼                   ▼
                    │    [YES]                 [NO]
                    │          │                   │
                    │          │                   ▼
                    │          │          ┌─────────────────────┐
                    │          │          │ PLAN STAYS IN       │
                    │          │          │ "scheduled" STATUS   │
                    │          │          │ (Manual scheduling)  │
                    │          │          └─────────────────────┘
                    │          │
                    │          ▼
                    │  ┌─────────────────────┐
                    │  │ CREATE FOLLOW-UP    │
                    │  │ APPOINTMENT         │
                    │  │ - parentAppointmentId│
                    │  │ - isFollowUp: true   │
                    │  │ - followUpReason    │
                    │  │ - status: SCHEDULED │
                    │  └─────────────────────┘
                    │          │
                    │          ▼
                    │  ┌─────────────────────┐
                    │  │ LINK PLAN TO        │
                    │  │ APPOINTMENT         │
                    │  │ - followUpAppointmentId│
                    │  │ - status: completed │
                    │  └─────────────────────┘
                    │          │
                    │          ▼
                    │  ┌─────────────────────┐
                    │  │ SEND NOTIFICATIONS  │
                    │  │ - Patient           │
                    │  │ - Doctor            │
                    │  │ - Reminders          │
                    │  └─────────────────────┘
                    │
                    ▼
            [END OF FLOW]
```

---

## 🔂 Recurring Appointment Series

```
┌─────────────────────────────────────────────────────────────────┐
│                  RECURRING APPOINTMENT SERIES                   │
└─────────────────────────────────────────────────────────────────┘

                    [CREATE RECURRING SERIES]
                              │
                              ▼
                    ┌─────────────────────┐
                    │  VALIDATE TEMPLATE  │
                    │  - Template exists  │
                    │  - Pattern valid    │
                    │  - Dates valid      │
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  CREATE SERIES      │
                    │  - seriesId         │
                    │  - startDate        │
                    │  - endDate          │
                    │  - status: active   │
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  GENERATE DATES     │
                    │  Based on pattern:  │
                    │  - WEEKLY           │
                    │  - MONTHLY           │
                    │  - DAILY            │
                    │  - CUSTOM           │
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  CREATE APPOINTMENTS│
                    │  FOR EACH DATE:     │
                    │  - seriesId         │
                    │  - seriesSequence   │
                    │  - status: SCHEDULED│
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  SERIES STRUCTURE   │
                    │                     │
                    │  RecurringSeries    │
                    │    ├── Appt 1       │
                    │    │   (seq: 1)     │
                    │    ├── Appt 2       │
                    │    │   (seq: 2)     │
                    │    ├── Appt 3       │
                    │    │   (seq: 3)     │
                    │    └── ...          │
                    │        (seq: N)     │
                    └─────────────────────┘
                              │
                              ▼
                    ┌─────────────────────┐
                    │  QUEUE NOTIFICATIONS│
                    │  FOR ALL APPOINTMENTS│
                    └─────────────────────┘
                              │
                              ▼
                    [SERIES CREATED]
```

---

## 🔗 Appointment Chain Structure

```
┌─────────────────────────────────────────────────────────────────┐
│                    APPOINTMENT CHAIN STRUCTURE                  │
└─────────────────────────────────────────────────────────────────┘

                    ┌─────────────────────┐
                    │  ORIGINAL APPOINTMENT│
                    │  (app-1)             │
                    │  - type: CONSULTATION│
                    │  - date: 2024-01-15 │
                    │  - status: COMPLETED │
                    │  - parentId: null   │
                    └─────────────────────┘
                              │
                              │ parentAppointmentId
                              │
                    ┌─────────┴─────────┐
                    │                   │
                    ▼                   ▼
        ┌───────────────────┐  ┌───────────────────┐
        │ FOLLOW-UP 1        │  │ FOLLOW-UP 2        │
        │ (app-2)           │  │ (app-3)            │
        │ - type: FOLLOW_UP │  │ - type: FOLLOW_UP  │
        │ - date: 2024-02-15│  │ - date: 2024-03-15 │
        │ - status: SCHEDULED│ │ - status: PENDING  │
        │ - parentId: app-1 │  │ - parentId: app-1  │
        │ - isFollowUp: true │  │ - isFollowUp: true │
        └───────────────────┘  └───────────────────┘
                    │                   │
                    │                   │
                    ▼                   ▼
        ┌───────────────────┐  ┌───────────────────┐
        │ FOLLOW-UP PLAN 1  │  │ FOLLOW-UP PLAN 2  │
        │ (plan-1)          │  │ (plan-2)          │
        │ - status: completed│ │ - status: scheduled│
        │ - appId: app-2    │  │ - appId: null      │
        └───────────────────┘  └───────────────────┘

CHAIN QUERY RESULT:
{
  original: {
    id: "app-1",
    date: "2024-01-15",
    status: "COMPLETED"
  },
  followUps: [
    {
      id: "app-2",
      date: "2024-02-15",
      status: "SCHEDULED",
      followUpPlan: { id: "plan-1", ... }
    },
    {
      id: "app-3",
      date: "2024-03-15",
      status: "PENDING",
      followUpPlan: { id: "plan-2", ... }
    }
  ]
}
```

---

## 📊 State Transitions

### Regular Appointment States

```
┌─────────────────────────────────────────────────────────────┐
│              REGULAR APPOINTMENT STATE MACHINE              │
└─────────────────────────────────────────────────────────────┘

    [PENDING]
        │
        │ (scheduled)
        ▼
    [SCHEDULED] ────────┐
        │               │
        │ (confirmed)   │ (cancelled)
        ▼               │
    [CONFIRMED]         │
        │               │
        │ (checked in)   │
        ▼               │
    [CHECKED_IN]        │
        │               │
        │ (started)      │
        ▼               │
    [IN_PROGRESS]       │
        │               │
        │ (completed)    │
        ▼               │
    [COMPLETED]         │
        │               │
        │               │
        └───────────────┘
              │
              │ (if followUpRequired)
              ▼
    [FOLLOW_UP_SCHEDULED]
```

### Follow-Up Plan States

```
┌─────────────────────────────────────────────────────────────┐
│                FOLLOW-UP PLAN STATE MACHINE                  │
└─────────────────────────────────────────────────────────────┘

    [scheduled] ────────┐
        │               │
        │ (appointment  │ (cancelled)
        │  created)     │
        ▼               │
    [completed]         │
        │               │
        │               │
        └───────────────┘
              │
              │ (if date passed)
              ▼
    [overdue]
```

### Follow-Up Appointment States

```
┌─────────────────────────────────────────────────────────────┐
│            FOLLOW-UP APPOINTMENT STATE MACHINE                │
└─────────────────────────────────────────────────────────────┘

    [FOLLOW_UP_SCHEDULED] (plan exists, not yet scheduled)
        │
        │ (appointment created)
        ▼
    [SCHEDULED]
        │
        │ (confirmed)
        ▼
    [CONFIRMED]
        │
        │ (checked in)
        ▼
    [CHECKED_IN]
        │
        │ (started)
        ▼
    [IN_PROGRESS]
        │
        │ (completed)
        ▼
    [COMPLETED]
        │
        │ (can create another follow-up)
        ▼
    [END]
```

---

## 🔄 Complete Flow: Consultation → Follow-Up

```
┌─────────────────────────────────────────────────────────────┐
│         COMPLETE FLOW: CONSULTATION WITH FOLLOW-UP          │
└─────────────────────────────────────────────────────────────┘

STEP 1: CREATE APPOINTMENT
──────────────────────────
POST /appointments
{
  patientId, doctorId, clinicId,
  appointmentDate: "2024-01-15T10:00:00Z",
  type: "CONSULTATION"
}
    │
    ▼
Appointment Created:
  - id: "app-1"
  - status: "SCHEDULED"
  - parentAppointmentId: null
  - isFollowUp: false


STEP 2: PATIENT CHECKS IN
──────────────────────────
POST /appointments/app-1/check-in
    │
    ▼
Status Updated:
  - status: "CHECKED_IN"
  - checkedInAt: "2024-01-15T10:00:00Z"


STEP 3: DOCTOR STARTS CONSULTATION
───────────────────────────────────
POST /appointments/app-1/start
    │
    ▼
Status Updated:
  - status: "IN_PROGRESS"
  - startedAt: "2024-01-15T10:15:00Z"


STEP 4: DOCTOR COMPLETES WITH FOLLOW-UP
────────────────────────────────────────
POST /appointments/app-1/complete
{
  followUpRequired: true,
  followUpDate: "2024-02-15",
  followUpType: "routine",
  followUpInstructions: "Monitor blood pressure"
}
    │
    ▼
Appointment Updated:
  - status: "COMPLETED"
  - completedAt: "2024-01-15T10:45:00Z"
    │
    ▼
FollowUpPlan Created:
  - id: "plan-1"
  - appointmentId: "app-1"
  - scheduledFor: "2024-02-15"
  - status: "completed" (auto-scheduled)
    │
    ▼
FollowUpAppointment Created:
  - id: "app-2"
  - parentAppointmentId: "app-1"
  - isFollowUp: true
  - followUpReason: "Monitor blood pressure"
  - date: "2024-02-15T10:00:00Z"
  - status: "SCHEDULED"
    │
    ▼
Plan Linked:
  - followUpAppointmentId: "app-2"


STEP 5: VIEW APPOINTMENT CHAIN
───────────────────────────────
GET /appointments/app-1/chain
    │
    ▼
Response:
{
  original: {
    id: "app-1",
    date: "2024-01-15",
    status: "COMPLETED"
  },
  followUps: [
    {
      id: "app-2",
      date: "2024-02-15",
      status: "SCHEDULED",
      parentAppointmentId: "app-1"
    }
  ]
}
```

---

## 📈 Database Relationship Diagram

```
┌─────────────────────────────────────────────────────────────┐
│              DATABASE RELATIONSHIP DIAGRAM                   │
└─────────────────────────────────────────────────────────────┘

Appointment
├── parentAppointmentId ──────┐
│                             │
│                             ▼
│                    Appointment (Parent)
│                             │
│                             │ parentAppointmentId
│                             │
│                             ▼
│                    Appointment[] (Follow-Ups)
│
├── seriesId ──────────────────┐
│                             │
│                             ▼
│                    RecurringAppointmentSeries
│                             │
│                             │ appointments[]
│                             │
│                             ▼
│                    Appointment[] (Series)
│
└── followUpPlan ─────────────┐
                              │
                              ▼
                    FollowUpPlan
                              │
                              │ followUpAppointmentId
                              │
                              ▼
                    Appointment (Follow-Up)
```

---

## 🎯 Decision Tree: Follow-Up Creation

```
┌─────────────────────────────────────────────────────────────┐
│            DECISION TREE: FOLLOW-UP CREATION                 │
└─────────────────────────────────────────────────────────────┘

                    [Appointment Completed]
                              │
                              ▼
                    ┌─────────────────────┐
                    │ followUpRequired?   │
                    └─────────────────────┘
                              │
                    ┌─────────┴─────────┐
                    │                   │
                    ▼                   ▼
              [NO]                 [YES]
                    │                   │
                    │                   ▼
                    │          ┌─────────────────────┐
                    │          │ Create FollowUpPlan │
                    │          └─────────────────────┘
                    │                   │
                    │                   ▼
                    │          ┌─────────────────────┐
                    │          │ followUpDate         │
                    │          │ provided?            │
                    │          └─────────────────────┘
                    │                   │
                    │          ┌─────────┴─────────┐
                    │          │                   │
                    │          ▼                   ▼
                    │    [YES]                 [NO]
                    │          │                   │
                    │          │                   ▼
                    │          │          ┌─────────────────────┐
                    │          │          │ Plan Status:         │
                    │          │          │ "scheduled"          │
                    │          │          │ (Wait for manual    │
                    │          │          │  scheduling)        │
                    │          │          └─────────────────────┘
                    │          │
                    │          ▼
                    │  ┌─────────────────────┐
                    │  │ Create Follow-Up    │
                    │  │ Appointment         │
                    │  │ - parentId: app-1   │
                    │  │ - isFollowUp: true  │
                    │  └─────────────────────┘
                    │          │
                    │          ▼
                    │  ┌─────────────────────┐
                    │  │ Link Plan to        │
                    │  │ Appointment         │
                    │  │ - status: completed│
                    │  └─────────────────────┘
                    │
                    ▼
            [End]
```

---

## 📝 Summary

### Key Flows Documented

1. ✅ **Regular Appointment**: Complete lifecycle from creation to completion
2. ✅ **Follow-Up Appointment**: Auto and manual scheduling flows
3. ✅ **Recurring Series**: Creation and management of appointment series
4. ✅ **Appointment Chain**: Parent-child relationships and queries
5. ✅ **State Transitions**: All possible state changes
6. ✅ **Decision Trees**: Logic for follow-up creation

### Key Relationships

- **Parent-Child**: `parentAppointmentId` links appointments
- **Plan-Appointment**: `followUpAppointmentId` links plans to appointments
- **Series-Appointments**: `seriesId` and `seriesSequence` link series

### Key Statuses

- **Appointment**: PENDING → SCHEDULED → CONFIRMED → CHECKED_IN → IN_PROGRESS → COMPLETED
- **Follow-Up Plan**: scheduled → completed/cancelled/overdue
- **Follow-Up Appointment**: FOLLOW_UP_SCHEDULED → SCHEDULED → ... → COMPLETED

---

**Last Updated**: 2024-01-15




