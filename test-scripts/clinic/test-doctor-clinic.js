/**
 * DOCTOR Role Clinic Endpoints Test
 * Tests all clinic endpoints accessible to DOCTOR role
 *
 * Run with: node test-scripts/clinics/test-doctor-clinic.js
 */
...
async function runDoctorClinicTests() {
  logSection('DOCTOR Role Clinic Endpoints Test');
  ...
  for (const testName of testSuite) {
    const testFn = doctorClinicTests[testName];
    if (testFn) {
      try {
        await testFn(ctx);
        await wait(300);
...
runDoctorClinicTests().catch(error => {
