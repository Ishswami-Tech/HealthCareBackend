/**
 * Sequential Test Runner - All Auth Endpoints by Role
 * Runs all role-specific auth test suites
 *
 * Run with: node test-scripts/auth/test-all-auth-sequential.js
 */
...
const roleTests = [
  { name: 'PATIENT', script: 'test-patient-auth.js' },
  { name: 'DOCTOR', script: 'test-doctor-auth.js' },
  { name: 'RECEPTIONIST', script: 'test-receptionist-auth.js' },
  { name: 'CLINIC_ADMIN', script: 'test-clinic-admin-auth.js' },
];
...
async function runAllAuthTests() {
  logSection('Running All Auth Endpoint Tests by Role');
