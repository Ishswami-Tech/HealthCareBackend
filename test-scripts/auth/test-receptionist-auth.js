/**
 * RECEPTIONIST Role Auth Endpoints Test
 * Tests all authentication endpoints accessible to RECEPTIONIST role
 *
 * Run with: node test-scripts/auth/test-receptionist-auth.js
 */
...
async function runReceptionistAuthTests() {
  logSection('RECEPTIONIST Role Auth Endpoints Test');
  ...
  for (const testName of testSuite) {
    ...
    const testFn = receptionistAuthTests[testName];
    if (testFn) {
      try {
        await testFn(ctx);
        await wait(300);
...
runReceptionistAuthTests().catch(error => {
