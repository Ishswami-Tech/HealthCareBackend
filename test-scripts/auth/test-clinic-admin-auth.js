/**
 * CLINIC_ADMIN Role Auth Endpoints Test
 * Tests all authentication endpoints accessible to CLINIC_ADMIN role
 *
 * Run with: node test-scripts/auth/test-clinic-admin-auth.js
 */
...
async function runClinicAdminAuthTests() {
  logSection('CLINIC_ADMIN Role Auth Endpoints Test');
  ...
  for (const testName of testSuite) {
    ...
    const testFn = clinicAdminAuthTests[testName];
    if (testFn) {
      try {
        await testFn(ctx);
        await wait(300);
...
runClinicAdminAuthTests().catch(error => {
