/**
 * DOCTOR Role Notification Preferences Endpoints Test
 * Tests all notification preference endpoints accessible to DOCTOR role
 *
 * Run with: node test-scripts/notification-preferences/test-doctor-notification-preferences.js
 */
...
const doctorNotificationPreferenceTests = {
  ...
  // These should fail for DOCTOR (admin only)
...
async function runDoctorNotificationPreferenceTests() {
  logSection('DOCTOR Role Notification Preferences Tests');
  ...
  for (const [testName, testFn] of Object.entries(doctorNotificationPreferenceTests)) {
    try {
      await testFn(ctx);
      await wait(500);
    } catch (error) {
...
runDoctorNotificationPreferenceTests().catch(error => {
