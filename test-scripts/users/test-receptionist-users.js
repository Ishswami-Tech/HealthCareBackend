/**
 * RECEPTIONIST Role Users Endpoints Test
 * Tests all user management endpoints accessible to RECEPTIONIST role
 *
 * Run with: node test-scripts/users/test-receptionist-users.js
 */
...
async function runReceptionistUsersTests() {
  logSection('RECEPTIONIST Role Users Endpoints Test');
  ...
  for (const testName of testSuite) {
    const testFn = receptionistUsersTests[testName];
    if (testFn) {
      try {
        await testFn(ctx);
        await wait(300);
...
runReceptionistUsersTests().catch(error => {
