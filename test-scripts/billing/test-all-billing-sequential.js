/**
 * Sequential Test Runner - All Billing Endpoints by Role
 * Runs all role-specific billing test suites
 *
 * Run with: node test-scripts/billing/test-all-billing-sequential.js
 */
...
const roleTests = [
  { name: 'PATIENT', script: 'test-patient-billing.js' },
  { name: 'DOCTOR', script: 'test-doctor-billing.js' },
  { name: 'RECEPTIONIST', script: 'test-receptionist-billing.js' },
  { name: 'CLINIC_ADMIN', script: 'test-clinic-admin-billing.js' },
];
...
async function runAllBillingTests() {
  logSection('Running All Billing Endpoint Tests by Role');
...
runAllBillingTests().catch(error => {
