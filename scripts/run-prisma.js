...
try {
  if (command === 'migrate' || command === 'deploy') {
    ...
    execSync(
      `node node_modules/prisma/build/index.js migrate deploy --schema=${schemaPath} --config=${configPath}`,
      {
        stdio: 'inherit',
    ...
    log(`  Database: ${cleanDbUrl.replace(/:[^:@]+@/, ':****@')}`, 'cyan');
    execSync(
      `node node_modules/prisma/build/index.js db push --schema=${schemaPath} --config=${configPath} --accept-data-loss`,
      {
        stdio: 'inherit',
        cwd: path.join(__dirname, '..'),
        env: env,
