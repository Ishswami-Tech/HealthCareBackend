...
export class CircuitBreakerService {
  ...
  private circuitStates = new Map<string, { state: 'closed' | 'open' | 'half-open', failures: number, lastFailureTime?: number }>();

  async execute<T>(
    fn: () => Promise<T>,
    options: CircuitBreakerOptions
  ): Promise<T> {
    const state = this.circuitStates.get(options.name) || { state: 'closed', failures: 0 };
