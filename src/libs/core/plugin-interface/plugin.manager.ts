...
export class EnterprisePluginManager implements PluginManager {
   ...
   * Mark plugin as unhealthy
   */
  private async markPluginUnhealthy(
    pluginName: string,
    errorMessage: string,
  ): Promise<void> {
    try {
      const healthResult =
        await this.pluginRegistry.getPluginHealth(pluginName);
      if (
        healthResult &&
  ...
  async executePluginsWithFallback(
    ...
    try {
      ...
      this.logger.warn(
        `⚠️ Primary plugin ${primaryPlugin} failed, trying fallbacks:`,
        error,
      );
      ...
      for (const fallbackPlugin of fallbackPlugins) {
        try {
          ...
          this.logger.warn(
            `⚠️ Fallback plugin ${fallbackPlugin} also failed:`,
            fallbackError,
          );
        }
      }
