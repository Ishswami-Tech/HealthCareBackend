...
export class QueueStatusGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      client.emit('connection_error', { error: error instanceof Error ? (error as Error).message : 'Unknown error' });
  ...
  async handleDisconnect(client: Socket) {
    ...
    if (session) {
      ...
      this.logger.log(`👋 Client disconnected: ${client.id} (tenant: ${session.tenantId})`);
    }
  }
  ...
  async handleGetQueueMetrics(
    ...
    try {
      ...
      const accessibleQueues = await this.getAccessibleQueues(session.tenantId, queueNames);
      ...
      client.emit('metrics_error', { error: error instanceof Error ? (error as Error).message : 'Unknown error' });
    }
  }
  ...
  private extractTenantId(client: Socket): string {
    return client.handshake.query.tenantId as string || 
  ...
  private async validateClientAccess(client: Socket, tenantId: string): Promise<void> {
  ...
  private async validateQueueAccess(tenantId: string, queueName: string): Promise<void> {
  ...
  private async getAccessibleQueues(tenantId: string, requestedQueues?: string[]): Promise<string[]> {
    const allStatuses = await this.queueService.getAllQueueStatuses();
    ...
    const healthcareQueues = allQueues.filter(q => 
      !q.includes('clinic') || q.includes('shared')
    );
