...
async function generateSchema() {
  try {
    ...
    logger.log(
      "npx prisma migrate dev --schema=./src/libs/infrastructure/database/prisma/schema.prisma",
    );

    return 0;
  } catch (_error) {
    logger.error("Failed to generate Prisma schema", (_error as Error).stack);
    return 1;
  }
}
