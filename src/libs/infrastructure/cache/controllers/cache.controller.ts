...
export class CacheController {
    ...
    schema: {
      ...
      properties: {
        ...
        debug: {
          type: "object",
          description:
            "Detailed debug information (only included if includeDebug=true)",
        },
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      "Update cache configuration settings including TTLs and rate limits.",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  async benchmarkCache(
    @Query("operations", new DefaultValuePipe(1000), ParseBoolPipe)
    operations: number,
    @Query("payloadSize", new DefaultValuePipe(1024), ParseBoolPipe)
    payloadSize: number,
  ) {
    ...
    return {
      ...
      operationsPerSecond:
        Math.round(((operations * 3) / totalDuration) * 1000 * 100) / 100,
      averageLatency:
        Math.round((totalDuration / (operations * 3)) * 100) / 100,
      operations: {
        set: {
          duration: setDuration,
          operationsPerSecond:
            Math.round((operations / setDuration) * 1000 * 100) / 100,
          averageLatency: Math.round((setDuration / operations) * 100) / 100,
        },
        get: {
          duration: getDuration,
          operationsPerSecond:
            Math.round((operations / getDuration) * 1000 * 100) / 100,
          averageLatency: Math.round((getDuration / operations) * 100) / 100,
        },
        del: {
          duration: delDuration,
          operationsPerSecond:
            Math.round((operations / delDuration) * 1000 * 100) / 100,
