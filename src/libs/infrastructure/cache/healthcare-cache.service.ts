...
@Injectable()
export class HealthcareCacheService implements OnModuleInit {
  private readonly logger = new Logger(HealthcareCacheService.name);
  ...
  async onModuleInit() {
    this.logger.log('Healthcare Cache Service initialized with HIPAA-compliant patterns');
  }
