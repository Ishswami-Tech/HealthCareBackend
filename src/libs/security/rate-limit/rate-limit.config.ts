...
export class RateLimitConfig {
  private readonly defaultLimits: RateLimitRule = {
    maxRequests: 1000000, // 1M requests for enterprise scale
  ...
  private readonly limits: Record<string, RateLimitRule> = {
    ...
    // Login attempts (enterprise scale)
    'auth/login': {
      maxRequests: 1000, // 1K login attempts
      windowMs: 60000, // 1 minute
      blockDuration: 300000 // 5 minutes block after limit exceeded
    },
    
    // Password reset requests (enterprise scale)
    'auth/password-reset': {
      maxRequests: 100, // 100 password reset requests
      windowMs: 3600000, // 1 hour
      blockDuration: 3600000 // 1 hour block
    },
    
    // OTP verification (enterprise scale)
    'auth/verify-otp': {
      maxRequests: 500, // 500 OTP verifications
      windowMs: 300000, // 5 minutes
      blockDuration: 900000 // 15 minutes block
    },
    
    // Token refresh (enterprise scale)
    'auth/refresh': {
      maxRequests: 10000, // 10K token refreshes
      windowMs: 300000, // 5 minutes
      blockDuration: 600000 // 10 minutes block
    },
    
    // Social login (enterprise scale)
    'auth/social': {
      maxRequests: 500, // 500 social logins
      windowMs: 60000, // 1 minute
      blockDuration: 300000 // 5 minutes block
    },
    
    // Magic link requests (enterprise scale)
    'auth/magic-link': {
      maxRequests: 100, // 100 magic link requests
      windowMs: 3600000, // 1 hour
