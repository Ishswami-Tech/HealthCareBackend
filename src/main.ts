...
async function bootstrap() {
  ...
  try {
    ...
    const port =
      configService.get("PORT") || configService.get("VIRTUAL_PORT") || 8088;
    ...
    const bullBoardUrl =
      configService.get("BULL_BOARD_URL") || "/queue-dashboard";
    ...
    try {
      ...
      logger.log(
        `Application is running in ${envConfig.app.environment} mode:`,
      );
                ...
                if (httpServer && typeof httpServer.close === "function") {
                  await new Promise<void>((resolve) => {
                    const timeout = setTimeout(() => {
                      logger.warn(
                        "WebSocket server close timeout, continuing...",
                      );
                      resolve();
                    }, 3000);
    ...
    if (loggingService) {
      try {
        ...
        console.error(
          "CRITICAL: Failed to log through LoggingService:",
          logError,
        );
      }
    } else {
