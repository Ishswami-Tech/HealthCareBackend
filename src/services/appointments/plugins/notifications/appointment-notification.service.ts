...
export class AppointmentNotificationService {
  ...
  /**
   * Send daily appointment summary WhatsApp to all doctors
   */
  async sendDoctorDailySummary(
    ...
    try {
      // Find all doctors with notification preferences that allow WhatsApp
      const doctors = await this.databaseService.executeHealthcareRead<
        Array<{ id: string; userId: string; specialization: string; clinicId: string }>
      >(async client => {
      ...
      for (const doctor of doctors) {
        try {
          ...
          >(async client => {
            const typedClient = client as unknown as PrismaTransactionClientWithDelegates;
            return (await typedClient.appointment.findMany({
              where: {
                doctorId: doctor.id,
          ...
          const appointmentSummaries = appointments.map(apt => ({
            ...
            date: formatDateKeyInIST(apt.date),
            time: apt.time ? formatTimeInIST(new Date(`1970-01-01T${apt.time}`)) : 'TBD',
            appointmentType: apt.type || 'consultation',
          }));

          const message = this.buildDoctorDailySummaryMessage(
            doctorName,
            clinicName,
            appointmentSummaries
          );

          const result = await this.whatsAppService.sendCustomMessage(
            doctorUser.phone,
            message,
          ...
          await this.loggingService.log(
            LogType.NOTIFICATION,
            LogLevel.INFO,
            `Doctor daily summary ${result ? 'sent' : 'failed'}`,
            'AppointmentNotificationService.sendDoctorDailySummary',
            { doctorId: doctor.id, doctorPhone: doctorUser.phone, count: appointments.length }
          );
        } catch (error) {
          failed++;
          await this.loggingService.log(
            LogType.ERROR,
            LogLevel.ERROR,
            `Failed to send daily summary to doctor: ${doctor.id}`,
            'AppointmentNotificationService.sendDoctorDailySummary',
      ...
      await this.loggingService.log(
        LogType.NOTIFICATION,
        LogLevel.INFO,
        `Doctor daily summary job completed`,
        'AppointmentNotificationService.sendDoctorDailySummary',
      ...
      await this.loggingService.log(
        LogType.ERROR,
        LogLevel.ERROR,
        `Doctor daily summary job failed`,
        'AppointmentNotificationService.sendDoctorDailySummary',
        { error: error instanceof Error ? error.message : String(error) }
      );
