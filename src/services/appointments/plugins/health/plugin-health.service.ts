...
export class PluginHealthService {
  ...
  async getPluginHealthMetrics(pluginName: string): Promise<PluginHealthMetrics> {
    try {
      ...
      const pluginInfo: unknown[] = []; // Mock plugin info array since registry is not available
      const plugin = pluginInfo.find(p => (p as Record<string, unknown>)['name'] === pluginName);
      ...
      this.logger.error(`Failed to get plugin health metrics for ${pluginName}:`, _error);
  ...
  async getDomainPluginHealth(domain: string): Promise<PluginHealthMetrics[]> {
    try {
      ...
      this.logger.error(`Failed to get domain plugin health for ${domain}:`, _error);
      return [];
    }
  }
  ...
  async updatePluginMetrics(
    ...
    try {
      ...
      this.logger.error(`Failed to update plugin metrics for ${pluginName}:`, _error);
    }
  }
