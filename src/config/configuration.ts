...
function parseBoolean(value: string | undefined, defaultValue: boolean): boolean {
  return parseEnvVar(value, defaultValue, val => val.toLowerCase() === 'true');
}
...
export default function createConfiguration(): Config {
  ...
  return {
    app: {
      ...
      environment:
        (process.env[ENV_VARS.NODE_ENV] as 'development' | 'production' | 'test') ||
        DEFAULT_CONFIG.ENVIRONMENT,
      isDev: process.env[ENV_VARS.NODE_ENV] !== 'production',
      host: process.env['HOST'] || 'localhost',
      bindAddress: process.env['BIND_ADDRESS'] || 'localhost',
      baseUrl:
        process.env['BASE_URL'] || `http://localhost:${process.env['PORT'] || DEFAULT_CONFIG.PORT}`,
      apiUrl:
        process.env['API_URL'] || `http://localhost:${process.env['PORT'] || DEFAULT_CONFIG.PORT}`,
    },
    urls: {
    ...
    jwt: {
      secret: process.env[ENV_VARS.JWT_SECRET] || 'your-secret-key-change-in-production',
    ...
    rateLimit: {
      ttl: parseInteger(process.env['RATE_LIMIT_TTL'], DEFAULT_CONFIG.RATE_LIMIT_TTL),
      max: parseInteger(process.env['RATE_LIMIT_MAX'], DEFAULT_CONFIG.RATE_LIMIT_MAX),
    },
    logging: {
      level:
    ...
    cors: {
      origin: process.env[ENV_VARS.CORS_ORIGIN] || 'http://localhost:3000,http://localhost:8088',
    ...
    security: {
      ...
      rateLimitWindowMs: parseInteger(process.env['SECURITY_RATE_LIMIT_WINDOW_MS'], 15000),
